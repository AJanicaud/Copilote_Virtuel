...
check_lists = xlrd.open_workbook('Check_lists_DR400.xlsx') # Flight Check lists
airfields_database = xlrd.open_workbook('airfields_database.xlsx') # Airfields databases

#check_lists = pd.read_excel('Check_lists_DR400.xlsx',encoding='utf-8')
...
def print_check_list(current_check_list) :
    ...
    while i<5 :
        ...
        Title1 = Label(Check_list, text=current_cl.col_values(0)[0], pady=3, height=2, font=font1, fg='red')
        Title1.pack()
     ...
     # for i in range (1, 9) :
        ...
        # Previous task
        Label(prev_task, text=current_cl.col_values(2)[i-1], font=font_aircraft).place(x=30, y=5)

        #Current Task
        Label(current_task, text=current_cl.col_values(2)[i], font=font_aircraft).place(x=30, y=5)
        Label(current_task, text="To do", font=font_aircraft).place(x=30, y=40)
        Label(current_task, text=current_cl.col_values(3)[i], font=font_aircraft).place(x=100, y=60)
    
        #Next task
        Label(next_task, text=current_cl.col_values(2)[i+1], font=font_aircraft).place(x=30, y=5)
